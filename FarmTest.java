...
class FarmTest {
	...
	void setUp() throws Exception {
		...
		Farm testFarm = new Farm(testFarmer, "Earth");
	}
	...
	@Test
	final void testSpendMoney() {
		Farm testFarm = new Farm(new Farmer(), "Test");
		testFarm.setMoney(100);
		testFarm.spendMoney(30);
		assertEquals(testFarm.getMoney(), 70);
		assertEquals(testFarm.getStartCash(), 100);
		try {
			testFarm.spendMoney(-3);
			fail("Should not accept negative integer as param");
		}
		catch (IllegalArgumentException e) {
			assertEquals(testFarm.getMoney(), 70);
		...
		try {
			testFarm.spendMoney(80);
			fail("Should not allow money to go below zero");
		}
		catch (IllegalArgumentException e) {
			assertEquals(testFarm.getMoney(), 70);
			assertEquals(testFarm.getStartCash(), 100);
